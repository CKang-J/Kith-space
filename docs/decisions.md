...
| 30 | Agent Harness v2 | per-surface session + durable delivery/logical turn/attempt + Context Envelope + revisioned episodic memory + broker-backed MCP/CLI Gateway；P-A10.0–P-A10.3 已完成，P-A10.4–P-A10.7 实施中 |
...
**状态**：Accepted / Implementing。P-A10.0–P-A10.3 已完成迁移、schema v6、Runtime v2 bridge、per-surface registry、durable turn、server-owned direct-mention thread、Context Envelope、实时父级ACL与turn inspector；P-A10.4完整Gateway、schema v7 memory和记忆管理UI尚未实施。
