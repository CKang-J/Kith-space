...
| CP-19 | 中 | 验证缺口 | Windows / Linux | Recombyn Canvas 阶段2的现有路径解析与文件打开会拒绝已存在的 symlink/junction/reparse-style link，并以 `fstat` 绑定本次读取的文件；resolver 每次从同一已打开 fd 读取并核验数据库 size + SHA-256。Node 没有跨平台 handle-relative unlink/unlinkat，逐段 `lstat`/`realpath` 也不能证明已消除并发父目录交换，因此阶段2删除只提交 DB tombstone，恢复与删除均不物理 unlink 资产或未知 staging；不可达文件留待阶段5平台 Adapter/GC。恢复分别校验 final 与 staging；若 final 已损坏而 staging 完整，则把已验证 bytes 写入服务端生成的 O_EXCL sibling、fsync 后以读取时的 storage key/state/deletedAt 做 DB CAS，旧 partial final 与 staging 均保留。写入 sibling 后的崩溃或竞争删除只产生安全 orphan，不覆盖、误指向或复活资产。macOS 已运行父目录交换、不物理删除、篡改读取 fail-closed 与离线删除重连 smoke；Windows/Linux 尚未实机验证。 | Windows/Linux CI 重放 schema v11→v12、junction/reparse/symlink 与父目录交换、逐次 hash 读取、崩溃窗口和恶意输入；阶段5用有可靠句柄删除语义的平台 Adapter 实现物理 GC，并做三端文件占用、长路径、Desktop GPU/输入 smoke。 |
| CP-20 | 中 | 性能告警 | Windows / macOS / Linux | Canvas 已 lazy-load，不再把完整 Recombyn editor 放进初始 shell chunk；当前生产构建的独立 Canvas chunk 仍约 3.5 MB，Vite 持续报告 >500 kB，lottie-web 还报告 `eval` 告警。功能构建通过不代表性能/CSP门禁通过。 | 阶段5基于真实打开时延、内存和 CSP 证据拆分 worker/导出/媒体能力；不得仅调高 warning threshold。 |
