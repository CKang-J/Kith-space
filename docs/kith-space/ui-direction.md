...
- Settings新增安装级“记忆 Advisor”页：分开展示聊天runtime之外的Provider revision与Advisor Model Profile revision，支持内置Pi/Claude切换、Pi CLI纯数据导入、模型/API/thinking/endpoint/凭据来源/data policy/allowed egress配置、能力探测、helper/Claude/隔离诊断、最近Provider Run审计和显式legacy回滚。目录中“不兼容但可见”的模型不可选择。Agent详情的Memory Advisor卡片显示实际系统Provider/model，并以逐Agent按钮授权或撤销公开频道、私密频道/话题与DM source scope；Provider、模型、凭据身份、目的地或策略变化后原授权失效。
...
- 系统级Advisor已在“设置 → Memory Advisor”落地安装级控制页：Provider选择为“Pi SDK（内置，fresh默认）/ Claude Code”，模型设置独立选择供应商、模型、API、endpoint、thinking level与凭据来源，并显示实际数据目的地、能力探测、执行审计和需重新授权的Agent数量。Pi CLI导入只能由Human点击触发，只展示脱敏的全局模型目录、默认模型、来源digest/时间和警告；导入结果先预览、确认后才生成不可变Model Profile，不读取项目`.pi`资源，也不执行命令表达式。Agent详情仍只负责本Agent enabled/paused、预算、pending、精确scope consent与当前系统Provider/模型摘要，不增加每Agent模型覆盖。没有模型/凭据/能力探测/授权时，“默认Pi”显示为需要设置且0 evidence外发。完整实现契约见`../superpowers/specs/2026-07-22-system-memory-advisor-provider-design.md`。
