...
P-A10.3已经让Human顶层direct mention的root直接带唯一话题，required持久回复只能进入该话题；Agent消息hover工具栏新增工作轮次入口，右侧审计抽屉以“上下文/步骤/用量/结果”展示冻结manifest、source状态/HMAC tombstone、attempt event/usage、逐input obligation与operation/output。MessageContextSnapshot只保存规范route和对象ref。P-A10.4新增的是runtime侧MCP/CLI Gateway、临时附件绑定、checklist/short wake和body-free manual inbox summary，没有新增Human可见面板；成功reply的附件继续复用既有Chat附件卡片。P-A10.2既有scoped activity/trajectory和ephemeral placeholder保持不变。session诊断与结构化记忆面板仍分别属于P-A10.5–P-A10.7，不能写成当前行为。完整机制、默认值和实施切片见 `../superpowers/specs/2026-07-19-agent-harness-session-context-memory-tools-design.md`。
