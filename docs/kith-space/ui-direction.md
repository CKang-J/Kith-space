...
本文是 Kith-space 当前 UI 信息架构与视觉语言的权威说明。单窗口交互契约见 `docs/archive/specs/2026-07-10-kith-space-single-window-workspace-design.md`，个人 AgentOS 宿主与产品边界见 `docs/archive/specs/2026-07-11-personal-agent-os-local-pivot-design.md`，Home/Space root 补充设计见 `docs/archive/specs/2026-07-12-home-space-and-space-root-design.md`；最新图标导航栏、消息中栏和消息气泡规格见 `docs/archive/specs/2026-07-23-chat-icon-rail-message-pane-design.md`，其覆盖 `docs/archive/specs/2026-07-15-chat-shell-sidebar-module-navigation-design.md` 的左侧栏视觉结构；模型与供应商、运行器、Memory Advisor和Agent记忆页的实现覆盖见`docs/archive/specs/2026-07-23-model-provider-runtime-memory-settings-design.md`。现有可交互线框 `docs/archive/historical/prototypes/kith-space-single-window-flow.html` 只覆盖早期三态壳，不代表最新导航目标。
...
完整边界、MVP 和后续能力见 `../archive/specs/2026-08-15-recombyn-canvas-workspace-design.md`。
...
- 系统级Advisor当前已在“设置 → Memory Advisor”落地安装级控制页；Provider隔离、Pi SDK/Claude选择、逐Agent精确consent、目的地预检和Run审计继续以`../archive/specs/2026-07-22-system-memory-advisor-provider-design.md`为安全事实。2026-07-23 后续规格把模型与CLI导入迁出该页，改由“模型与供应商”提供共享配置，Advisor页只选择执行器和模型配置；Agent记忆页也从完整控制卡改为摘要条与管理抽屉。完整覆盖见`../archive/specs/2026-07-23-model-provider-runtime-memory-settings-design.md`。
