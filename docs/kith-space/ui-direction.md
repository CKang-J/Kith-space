...
全局字体尺度使用11/12/14/16/20px五档 Token，正文基准为14px；元数据使用11–12px，控件或强调层使用16px，页面标题使用20px，避免业务视图散落新的正文基准。
...
- 全局 40px 顶部栏已退役；Space 快速切换入口位于图标栏顶部，Messages / Search / Spaces / Inbox / Tasks / Agents / Settings 只显示图标，并通过 hover/focus tooltip 显示名称。标题、导航、正文和元信息统一使用系统无衬线字体。
...
- 活跃会话行使用低饱和 `#f1f6fc` 浅蓝表面，“已保存”与普通会话保持相同层级，不使用额外阴影或强调底块；Human 消息使用 `#e7f0fe` 并右对齐，Agent 消息使用 `#f7f8fa` 并左对齐。气泡使用16px 18px内距和1.68正文行高，日期、时间与话题最新回复统一为12px低对比元数据。Markdown 标题限制在气泡阅读尺度，代码块使用带复制入口的深色独立表面，表格只在气泡内部横向滚动，链接仅在hover时显示下划线。
...
- Settings 复用既有 Human、Space、模型与供应商、运行器、自动整理记忆和 Desktop 内容，不复制第二套设置表单。
