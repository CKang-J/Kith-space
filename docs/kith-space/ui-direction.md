...
- 尚未实现的系统级Advisor提案在“设置 → Memory Advisor”增加安装级控制页：Provider选择为“Pi SDK（内置，默认）/ Claude Code”，模型设置独立选择供应商、模型、API、endpoint、thinking level与凭据来源，并显示实际数据目的地、能力探测和需重新授权的Agent数量。Pi CLI导入必须由Human点击触发，只展示脱敏的全局模型目录、默认模型、来源digest/时间和警告；导入结果先预览、确认后才生成不可变Model Profile，不读取项目`.pi`资源，也不执行命令表达式。Agent详情仍只负责本Agent enabled/paused、预算、pending、consent与当前系统Provider/模型摘要，不增加每Agent模型覆盖。没有模型/凭据/授权时，“默认Pi”显示为需要设置而不是伪装成运行中。完整目标态见`../superpowers/specs/2026-07-22-system-memory-advisor-provider-design.md`。
