...
- 当前已实现的“记忆 Advisor”页仍把Provider切换、模型Profile、Pi导入、能力诊断和Provider Run平铺在同一页面。2026-07-23 已接受的后续信息架构新增“模型与供应商”和“运行器”：前者统一管理与runtime解耦的供应商连接、可复用模型配置和只读CLI导入，后者管理Claude Code/Codex/OpenCode/Pi的安装状态、版本、三态默认绑定与高级参数；Kith默认不写任何CLI全局配置，而在子进程启动时编译注入。全新安装按“连接供应商→创建模型配置→设置运行器默认→返回创建Agent”引导并保留草稿；Web敏感动作给出Desktop恢复路径。
- 后续“记忆 Advisor”页只保留启用状态、Pi SDK（内置记忆执行器）/Claude执行器、一个兼容模型配置、真实数据目的地、测试和Agent授权影响；revision/epoch/digest、隔离细节和最近Provider Run进入默认收起的诊断抽屉。授权管理以逐Agent任务列表展示来源范围、目的地变化与失效原因。现有Advisor Model Profile降为内部不可变执行快照，不再让普通用户直接编辑。Agent详情把完整Memory Advisor卡片收敛为单行摘要与管理抽屉，结构化记忆列表/详情占据主体；状态、搜索与单一“筛选”菜单同行，类型/范围以active chips显示。主从断点基于模块内容容器而非浏览器viewport，必须覆盖侧栏/Dock、200% zoom和长文案。
...
- Pi 当前在runtime目录中可见且已有experimental one-shot adapter，但尚未进入P-A10 v2强路径。后续把Pi提升为第四个正式runtime，用户可见名为“Pi Agent（本机CLI）”，与“Pi SDK（内置记忆执行器）”严格分开；运行器页分别显示未安装、版本过旧、不支持RPC或可用。聊天执行改用本机外部Pi CLI RPC模式，按surface隔离session并接入durable turn、Context Envelope、Kith CLI Gateway、usage、cancel、snapshot和compaction telemetry；Pi没有内置MCP时必须显示CLI Gateway而不是伪报MCP。
