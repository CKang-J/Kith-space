...
- 设置现已新增“模型与供应商”和“运行器”：前者统一管理与runtime解耦的供应商连接、可复用模型配置和只读CLI导入，后者管理Claude Code/Codex/OpenCode/Pi的安装状态、版本、三态默认绑定与高级参数；Kith默认不写任何CLI全局配置，而在子进程启动时编译注入。创建Agent保留草稿；Web敏感动作给出Desktop恢复路径。Agent详情由独立模型绑定编辑器呈现`ready / confirmation_required / setup_required / restart_required`，允许Human重新确认或改为固定配置；未恢复`ready`前服务端拒绝启动，界面不得把“重启”冒充“重新确认”。
- “记忆 Advisor”页只保留启用状态、执行器、一个兼容模型配置、真实数据目的地和Agent授权影响；revision/epoch/digest、隔离细节和最近Provider Run进入默认收起的诊断区。Advisor Model Profile降为内部不可变执行快照，不再让普通用户直接编辑。
