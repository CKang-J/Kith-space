...
Canvas 是新的深 Module，不是 `WorkspaceFrame`、Renderer Redux 或 Agent runtime 的附属状态。阶段3依赖方向为 Human REST 与 React adapters → `src/canvas/` Core/Import/Asset 边界，以及 MessagePosting / Context Assembler 的窄 Selection Snapshot seam；阶段4把 MCP/CLI thin tools、`CanvasAccessGrant` 和 Agent capability 接到同一 `CapabilityGateway`/`CanvasCore`，生产入口自决策 40 起默认开启，`KITH_CANVAS_AGENT_EXECUTION=0|false|off` 是整体退出开关（含打包产物）。Transport 只处理认证、限额、解析和序列化；业务 mutation、asset 生命周期、revision、幂等和冲突都位于 `src/canvas/`，不得继续扩大 `src/server/core.ts` 的职责。
