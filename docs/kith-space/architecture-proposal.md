...
> 本文描述个人 AgentOS 的目标模块边界。A2-A6、Home/Space root 的 H1-H4、P-A8、本轮聊天/壳层 UI、P-A9.0-P-A9.7、P-A10.0–P-A10.7、系统Memory Advisor Provider切片0–4与统一模型/运行器控制面均已完成。当前 workspace schema v12、app.db v10、per-surface SessionModule、四家 v2 runtime、durable delivery/turn、Context Envelope、broker-backed MCP/CLI Gateway、revisioned episodic memory、安装级模型/运行器/Advisor控制面、外观设置、带来源归属的Agent活动历史，以及 Human-only Recombyn Canvas Core 已落地。H5继续作为独立后续。Canvas 阶段2已完成并通过主任务最终复核；Selection Snapshot、Chat/Gateway和Agent写回仍未进入，见本文第14节与对应规格。
...
## 14. Recombyn Canvas Module（阶段 2 Human 本地闭环已实现）
...
阶段2每个 Space 实际新增 `canvas_documents`、`canvas_mutations`、`canvas_assets`；`canvas_selection_snapshots`、`canvas_access_grants`、`message_execution_bindings` 与 `turn_output_artifacts` 是阶段3/4目标，当前不存在。SceneDocument 作为不含内联大媒体的 canonical JSON 保存；aggregate revision 用于 snapshot/reconnect，mutation ledger 中服务端派生的资源 read/write set 用作 element/Frame/structure 细粒度 CAS，允许同一 base revision 上互不相交的资源提交。Canvas 资产使用 `<spaceRoot>/.kith/canvas-assets/<canvasId>/<assetId.ext>`，不复用聊天附件的 `.kith/uploads` 生命周期；路径只由已注册 Space root、Canvas id 和服务端生成的 asset id 解析，调用方不能提交任意文件路径。资产 reachability 包含当前 scene 与可由 retained inverse 恢复的历史；当前尚无有界 retention/checkpoint，因此历史存储和曾引用资产的回收会线性增长，属于未通过的长期性能/生命周期门禁。
