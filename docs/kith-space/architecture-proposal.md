...
Recombyn RCB/editor/nodes/chrome 以 `web/src/features/canvas/upstream/` 的 source island 迁入并保留内部 UI；feature-local Redux 只保存未提交手势、乐观交互投影与缓存，不能维护或恢复 canonical history。已提交 Ctrl-Z/Ctrl-Y 必须向 Core 提交新的 revert/forward mutation，Yjs undo 路径不迁入。所有 durable 写先变成 `CanvasMutationCommand`；Core 的 `analyzeCanvasOperationBatch()` 从 operation schema 推导不可少报/多报的 read/write/root/order/asset set，并在同一 workspace SQLite transaction 中完成授权、metadata/document/element/Frame/structure revisions 校验、apply、revision bump、mutation ledger 与 per-Canvas realtime sequence。Agent 幂等复用既有 turn operation 域，Human 使用全局唯一 client command。Renderer ACK 只表示已投影，不能成为业务 commit。
...
外部 Scene JSON 不得直接替换 canonical document。Kith-owned `CanvasImportService` 负责 schema/大小/深度/数量门禁、ID 重映射、保留 metadata 剥离、单隐藏 root 归一、asset rebinding、URL/data URL 拒绝和 SVG 清洗，最终只产出正常 Core operations 并经过同一 analyzer/transaction。
