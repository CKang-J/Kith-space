...
> 本文描述个人 AgentOS 的目标模块边界。A2-A6、Home/Space root 的 H1-H4、P-A8、本轮聊天/壳层 UI、P-A9.0-P-A9.7、P-A10.0–P-A10.7、系统Memory Advisor Provider切片0–4与统一模型/运行器控制面均已完成。当前 workspace schema v15、app.db v11、per-surface SessionModule、四家 v2 runtime、durable delivery/turn、Context Envelope、broker-backed MCP/CLI Gateway、revisioned episodic memory、安装级模型/运行器/Advisor控制面、外观设置、带来源归属的Agent活动历史，以及 Human-only Recombyn Canvas Core、阶段3 Selection Snapshot/Chat 联动、阶段4 Canvas Access Grant/Gateway 写回与 Phase 3.1–3.3 图像/视频生成 Job（Doubao 图像 / Seedance 视频适配器）已落地。H5继续作为独立后续。Canvas 阶段5硬化、真实供应商 smoke 与发行验收见对应规格。
