...
> 本文描述个人 AgentOS 的目标模块边界。A2-A6、Home/Space root 的 H1-H4、P-A8、本轮聊天/壳层 UI、P-A9.0-P-A9.7、P-A10.0–P-A10.7、系统Memory Advisor Provider切片0–4与统一模型/运行器控制面均已完成。当前 workspace schema v16、app.db v12、per-surface SessionModule、四家 v2 runtime、durable delivery/turn、Context Envelope、broker-backed MCP/CLI Gateway、revisioned episodic memory、安装级模型/运行器/Advisor控制面、外观设置、带来源归属的Agent活动历史，以及 Human-only Recombyn Canvas Core、阶段3 Selection Snapshot/Chat 联动、阶段4 Canvas Access Grant/Gateway 写回、Phase 3.1–3.3 图像/视频生成 Job（Doubao 图像 / Seedance 视频适配器）与 OpenRouter 音频 TTS 已落地。H5继续作为独立后续。Canvas 阶段5硬化、真实供应商 smoke 与发行验收见对应规格。
...
- Agent 轨迹展示由 `web/src/features/trajectory/` 统一建模：实时聚合面板和 Agents“活动”页都先把扁平事件转换为 `TrajItem`，再经 `groupTraj` 按 turn/stream、思考、正文和 `toolCallId` 关联工具开始/结束事件，最后交给共享 `TrajectoryTimeline`。展示层使用项目内接入的 assistant-ui Ghost `Reasoning`、`ToolFallback` 与 `ToolGroup` 组合组件，Shell 详情继续使用 AI Elements `Terminal`；思考在运行中自动展开、结束后自动折叠，命令与工具调用默认折叠并可查看输入、输出和失败态。assistant-ui 触发器只做适配本项目未启用按钮 reset 的局部无边框/透明背景复位及中文文案，不改写其状态图标、shimmer、箭头和折叠结构。聚合面板消费当前base conversation的实时缓冲，不重复来源；Agents活动页消费跨会话持久历史，每个turn携带`channelId / conversationId / streamId`并由服务端批量解析频道、Human-Agent私信或话题父消息摘要，前端只负责显示和深链导航。该层不按名称猜测来源，也不解析 Pi/Claude/Codex 的原始协议。
...
- P-A8 前端把默认设置与频道覆盖保持为两条清晰边界：`web/src/views/agent-response-mode/AgentDefaultResponseModeCard.tsx:14` 只修改当前 Space 的 Agent 默认值；`web/src/views/agent-response-mode/useChannelAgentResponseModes.ts:28` 只负责每频道一次装载、实时失效与频道覆盖写入。消息侧点击入口位于 `web/src/views/chat-message/AgentMessageCard.tsx:36`，从 `Chat.tsx` 接收已加载的 Agent/频道 DTO 和更新回调（`web/src/views/Chat.tsx:354`、`:897`），卡片本身不读取 Store、Router 或直接请求 API；其三段式控件只写当前频道覆盖，并提供恢复为 Agent 默认，绝不修改 Agent 默认值。话题复用父频道有效值，DM 不渲染频道模式，归档只读；`channel-settings/` 成员页不复制编辑器。Composer 的多 Agent 任务检查由 `web/src/views/composerTaskMentions.ts` 独立解析，频道全体 token 的识别与候选匹配由 `web/src/views/composerChannelAllMention.ts` 承担；动作菜单、附件预览和文本宽度驱动的紧凑/展开状态分别收口在 `web/src/views/composer/ComposerActions.tsx`、`ComposerAttachments.tsx` 与 `useComposerExpansion.ts`；Canvas pending 列表、eligible executor 拉取与发送 payload 收口在 `useComposerCanvasContext.ts` / `composerCanvasContext.ts`，`Composer.tsx` 只组合这些窄组件并保留发送语义。
...
- Worker 给 `agent:activity` / `agent:trajectory` 事件标记 `scoped | unscoped | ambiguous` 和实际 `channelId`；同步投递失败会用 schedule token 精确回滚自己的作用域，不能污染下一轮（`src/daemon/agentManager.ts:357`-`:383`、`src/daemon/trajectoryScope.ts:77`）。Core 对同一 Worker 连接的消息使用串行队列，确保 trajectory 数据先于随后到达的 terminal activity 边界处理（`src/server/ws.ts:55`、`src/server/workerMessageQueue.ts:1`）；随后校验该 channel 属于当前 Space，并把 thread 逐级归一为父 `conversationId`（`src/server/trajectoryScope.ts:61`）。Socket 透传作用域，Web Store 只把明确 `scoped` 事件写入 `trajByConversation[conversationId]` 的独立 300 条实时缓冲，Space 切换清空全部桶（`web/src/store.tsx`、`web/src/trajBuffer.ts`）。同一稳定作用域还随活动行写入workspace schema v11的`agent_activity_log.channel_id / conversation_id / stream_id`；`GET /api/channels/:conversationId/activity-log`通过Human频道可读权限后，从`src/server/conversationActivityHistory.ts`恢复该base conversation最近300条跨Agent轨迹，`web/src/views/LiveTrace.tsx`再与实时缓冲进行多重集去重合并。因此刷新、切换、重连或面板重新挂载不会丢失聚合轨迹，普通话题和server-owned thread的轨迹也随父`conversationId`恢复。跨会话Agents活动页继续批量解析并深链回原频道、私信或话题；旧直连频道行可按`channel_id`兼容恢复，其他旧行保持NULL并在UI诚实显示未记录。无作用域或 ambiguous 事件仍可进入 Agent 活动流，但不得出现在任何会话聚合面板。
- P-A10 v2 的 required turn 会在Core持有的server-owned thread surface执行；其逐条事件和terminal状态都必须先经过同一 `resolveTrajectoryScope`，再归一到父频道/DM。不得直接把 `runtime_sessions.surface_id` 同时当作 `conversationId`，否则运行详情虽已持久化，父会话聚合面板仍会漏掉整轮轨迹。
- Pi RPC v2 在 adapter 边界只把 `text_delta` 和 `thinking_delta` 投影为可见正文/思考摘要，忽略只代表未完成 JSON 参数的 `toolcall_delta`；`tool_execution_start/end` 以 `toolCallId` 携带有界的输入、输出和失败态。Worker 的250ms预览窗口按类型拼接增量而不是覆盖前一片段，`message_end` 仅在最终正文是已流式正文的严格后缀时补发缺失尾部，避免既截断又重复。Core 最多保存和广播16KiB轨迹正文或工具详情；旧 print-mode Pi 兼容路径使用同一16KiB预算，不再保留历史2KiB静默截断。
...
这些对象属于app.db，明文凭据继续只由CredentialPort持有，并加密保存在`<appData>/secrets/advisor-credentials.json`。普通文件、大小上限、同一FD读取前后身份与内容digest等门禁在三端保持不变；macOS/Linux通过`src/security/posixFileMetadata.ts`校验uid和group/world权限掩码，Windows通过`src/security/privateFileSecurity.ts`取得当前SID，先只读确认owner并仅在不符时独立修正；DACL通过.NET文件系统ACL API直接读取/写回Access section，不依赖PowerShell安全模块自动加载，保留安全描述符非DACL元数据且不重写未变化owner，再禁用继承、清空access rule并确定性写入当前owner唯一Allow规则，也不把Node合成的`st_mode`当成安全事实。任何其他主体的Allow ACE仍fail closed。既有Windows凭据会在首次读取时升级，任何收紧或复核失败都fail closed。运行器默认显式区分`kith_model_configuration | unmanaged_cli_native | unset`；CLI-native不可用于Advisor且不能冒充完整可审计配置。Space内Agent只保存`runtime_default | pinned`绑定意图、稳定配置ID/revision和最近由Human确认的非敏感provider/model/安装fingerprint；跨库不建立SQLite外键，目标机器缺失安装级配置或默认fingerprint变化时进入`confirmation_required/setup_required`，不静默使用该机器的其他默认模型。现有Advisor Model Profile保留为由Model Configuration编译出的内部不可变执行快照，继续服务job pinning、egress、epoch和consent，不再成为普通用户直接编辑的第二套模型对象。

Core新增窄`ModelProviderConnectionService`、`ModelConfigurationService`、`RuntimeProfileService`、`AdvisorBindingService`和脱敏presenter；Worker新增`RuntimeConfigCompilerRegistry`，按runtime把统一配置翻译为参数、child-only环境、Kith-owned临时配置和fingerprint。模型配置 presenter 现在优先读取同一条模型配置的 runtime 兼容快照来判断 Pi SDK / Claude Code Advisor 是否可用；这样 DeepSeek 这类 OpenAI 兼容配置只要快照允许 Pi，就会在记忆整理中可选。聊天runtime另有`RuntimeCredentialActivationPort`：Core只下发绑定session/generation/revisions/epoch/digest/expiry的无密钥descriptor，Worker通过Worker-only本机控制通道单次兑换，明文只短暂进入Worker内存和child env。安全相关配置变化先提升安装级`runtime_configuration_epoch`阻断旧admission，再撤销attempt/activation和关闭旧session，不能把异步`restart_required`当安全门。Claude Code使用启动参数/官方环境，Codex使用启动override或临时`CODEX_HOME`/profile，OpenCode继续使用`OPENCODE_CONFIG_CONTENT`。Pi 在 Kith 模型路径使用 compiler 注入的`PI_CODING_AGENT_DIR`；`unmanaged_cli_native`不覆盖该目录，session 仍隔离在 generation 的`--session-dir`。Kith可显式、只读导入Pi/Claude/Codex/OpenCode安全配置元数据，但默认不修改任何用户CLI全局文件；未来写回若存在，必须是独立、显式、diff/备份/原子/可回滚的高级动作。

Agent启动 admission 同样 fail closed：除合法legacy绑定外，解析结果不是`ready`时，Core在创建/恢复runtime session之前直接拒绝。Human必须在Agent详情的独立绑定编辑器中重新确认当前默认fingerprint或选择固定配置；确认后才创建携带当前epoch的新generation，避免`restart_required`状态被一次普通“启动”静默绕过。v2 Agent 创建同样 fail closed：`POST /api/agents` 在绑定解析不是 `ready` 时返回 409 `model_binding_setup_required`，不留下无法投递的僵尸 Agent。从未确认过目的地的 `runtime_default` Agent（fingerprint 为空）会在运行器默认从 `unset` 变为可用时同步为 ready 并写入当前 fingerprint；已经确认过不同 fingerprint 的 follow-default Agent 仍保持 `restart_required`，必须由 Human 再确认。Turn scheduler 在绑定仍阻塞时写入 `agent_activity_log`，不再静默丢弃 pending delivery。
...
app.db v6新增稳定连接、模型配置、runtime profile及其不可变revision、短寿命probe和脱敏CLI导入快照；v7 初始增加三类字体白名单字段，v8 将其迁入独立 `appearance_settings` 单例并允许界面字体选择无衬线或等宽分组，v9 在该单例增加受约束的12–16px UI字号，v10 再增加受约束的全局 light / dark / system 颜色模式，v11 增加 `generation_providers`，v12 把 name/type CHECK 扩到 `openrouter`/`audio`（`src/app-data/appDatabaseMigrations.ts:4`）。独立 `AppearanceSettingsService` 校验并部分更新（`src/appearance-settings/appearanceSettingsService.ts`），Human-authenticated `GET/PATCH /api/settings/appearance` 暴露安装级设置（`src/server/routes-api/appearanceSettings.ts:11`）。`GET/PATCH /api/settings/generation-providers` 暴露合并后的火山方舟 `ark` 凭据状态（内部仍写 `doubao`/`seedream` 两行，永不回明文 Key）以及独立的 `openrouter` 视图（`src/server/routes-api/generationProviderSettings.ts:17`）；写 Key 要求 Desktop trust 或本机 loopback，与模型供应商密钥同一门禁。Vite 开发代理保留浏览器可见 Host，使 Core 的 Origin/CSRF 与 WebSocket 同源门禁继续成立（`web/vite.config.ts:26`）。workspace.db v10增加Agent模型绑定、跨安装确认快照与session runtime epoch，同时保留旧runtime/model/runtime_config作迁移输入。`model-control/`拥有领域服务与脱敏presenter，`runtime/config/`拥有四家窄compiler、单次凭据activation和fail-closed epoch gate。配置revision变化提升安装级epoch，旧session admission立即失效并由新generation生效；默认不写任何CLI全局配置。
...
## 14. Recombyn Canvas Module（阶段 3 Chat 上下文联动已实现）

阶段 1 在 `web/src/features/canvas/` 建立隔离 Renderer feature island，固定320项闭包、318项 materialize 与两个 Stage1 SHA。阶段2保持 `upstream/**` 字节不变，经 host/adapters 与失败即停的编译期 Stage2 materializer 接入正式产品：`canvas` module、Canvas Library、按 Space 持久化的 resource tab，以及 `?module=canvas&canvas=<id>&canvasTitle=<title>` 已启用。原生 Redux 只保留手势/乐观投影；Core snapshot 是首次 hydrate 与断线恢复唯一来源，资源 generation、冻结 CAS 队列和单调 sequence 防止旧 Canvas/旧 revision 写回新资源。

workspace schema v15 在 v14 `canvas_access_grants` / `turn_output_artifacts` 之上新增 `canvas_generation_jobs`（`src/db/schema.ts` 的 `canvasGenerationJobs`，迁移 `drizzle/0016_canvas_generation_jobs.sql`）。v16 重建该表 CHECK，允许 `job_type='audio'` 与 `provider='openrouter'`（`drizzle/0017_canvas_generation_audio.sql`）。`src/canvas/generation/` 提供 Job 队列、Ark Doubao 图像供应商（`POST /images/generations`，同步 URL）、Seedance 视频供应商（规格 id `seedream`，`/contents/generations/tasks`）、OpenRouter 音频供应商（`POST /audio/speech`，同步 mp3 字节，`src/canvas/generation/providers/openrouterAudioProvider.ts:19`）、Worker 导入 `CanvasAssetStore` 后按 live revision 写入节点：Human 生成板带 `targetNodeId` 时就地 promote（同一 id，清 generator 标记并写 durable `attrs.src`），否则 `create_image`/`create_video`（音频内部可走 `create_audio` ToolOp；Gateway 尚未暴露 typed `canvas.create_audio`）。Human `POST /api/canvases/:canvasId/generation-jobs` 与 Agent Gateway 共用队列，不要求 Canvas Access Grant（`src/canvas/generation/humanGeneration.ts`，`src/server/routes-api/canvasGenerationJobs.ts`），入队后 `kickGenerationWorker` 立即 `pollOnce`（`src/canvas/generation/generationSupervisor.ts`）。图片选区工具条的放大/去背景/多角度经 host `processImageTool` 入队同一 Job，`placement.skipNodeCreate` 只导入结果资产、不另造节点；Doubao 提交时带参考图 `image` data URL，`arkFetchJson` 默认 90s 超时以免图生图阻塞 Worker 轮询。去背景模式菜单留在选区工具条内，不用 Floating UI portal。画布右键菜单经 `recombynReactDom` 挂到 island portal root（`web/src/features/canvas/adapters/recombynFloatingUi.tsx` 空层 `pointerEvents: none`），backdrop/panel/flyout 必须 `pointer-events-auto` 且用 portal 本地坐标（`CanvasContextMenu.tsx` / `recombynPortalMenuPosition.ts`），否则点击穿透到画布、或出现在视口坐标对应的错误位置（workspace tab 的 transform 动画会把 `position: fixed` 的包含块变成画布面板）。橡皮本地擦除后走 Canvas-asset 上传（`cloneSceneValue` 在 Immer reducer 内先 `current()` 再拷贝，避免 `structuredClone` 草稿 Proxy，`web/src/features/canvas/upstream/apps/web/src/components/rcb/scene/document/sceneDocument.ts:58`）；标记拖拽用 overlay 屏幕矩形换算视口坐标（`web/src/features/canvas/adapters/recombynMarkOverlay.ts`），松手后裁切 PNG 飞入左侧 Composer，框选矩形冻进 snapshot `projection.markedRegions` 只注入 Agent Envelope，不进 Human 正文（`kith:composer-attach-file` + 该图片节点的 `kith:canvas-selection-to-chat`），不再写入 Recombyn AgentDock 的 `pendingAgentContexts`；分层/detectRegions 仍拒绝。Agent ToolOp `image_process` 仍延后。生成器模型目录在 `src/canvas/generation/arkModelCatalog.ts`（Seedream 4.0/4.5/5.0 与 Seedance 1.0 Pro/Lite）；Human 模型选择器复用 Recombyn `ModelPickerPanel`/`ModelBrandIcon`，web 构建依赖 `@lobehub/icons-static-svg@^1.94.0` 经 Vite `?url` 打包静态 SVG（`web/src/features/canvas/upstream/apps/web/src/components/editor/panels/agent/ModelPickerPanel.tsx:24`），Seedream/Seedance 显示豆包标，OpenRouter TTS 分别显示 Gemini/OpenRouter/Fish Audio 标；`dreamina.png`/`sync_lipsync.png` 仍不复制。Human Job `config` 携带 `model`/`resolution`，Worker 按模型把图像 `size` 写成 K 档或 WxH、视频发送 `resolution`。`create_image` 接受本 Canvas `assetId` 或 `genPrompt`（入队返回 jobId，不是 mutation）；远程 URL/data URL 仍拒绝。媒体节点除 `assetId` 外必须写入编辑器可渲染的 durable `attrs.src`（`/api/canvas-assets/{spaceId}/{canvasId}/{assetId}`，与工具栏上传同一 resolver；`src/canvas/canvasToolOps.ts` 的 `durableCanvasAssetSrc`）。已落盘但缺 `src` 的节点由前端 `hydrateCanvasDocumentMediaSrc` 补全。app.db v11 `generation_providers` 存加密 Key；v12 允许 `openrouter`/`audio`。真实 Ark / OpenRouter 联调与 Desktop smoke 仍待本机验收。workspace schema v14 在 v13 `canvas_selection_snapshots` / `message_execution_bindings` 之上新增 `canvas_access_grants`、`turn_output_artifacts`，并为 binding 增加 `binding_source`。`src/canvas/canvasCore.ts` 在单一 SQLite transaction 内执行 operation、服务端派生的 element/Frame/Frame-membership/parent/root/order read/write 影响集、细粒度冲突检查、revision bump、幂等轻量 operation/inverse ledger、sequence 与 Core-owned undo/redo；所有服务端派生的真实 structure 影响共享同一冲突域，renderer 对既有 Frame 属性提交稳定 Frame ID patch，因此不相交普通 element/不同 Frame 纯属性可从同一 base 提交，而 membership、group/reparent、order 与 Frame 增删/重排会彼此冲突。`src/server/routes-api/canvas.ts` 的 `/changes` 对同 Space 已软删除 Canvas 返回 tombstone，离线客户端重连后幂等关闭 resource tab 并清 URL，网络错误和跨 Space 404 不映射为删除。`CanvasAssetStore` 将物理文件放在 `<space>/.kith/canvas-assets/<canvasId>/`，每次 resolver 读取从同一已打开 fd 核验 size + SHA-256；Node 没有跨平台 handle-relative unlink，因此删除只写 DB tombstone，恢复与删除均不物理 unlink 用户可变目录中的资产或未知 staging，物理 GC 延后阶段5。final 损坏但 staging 完整时，恢复把已验证 bytes 写入新的 O_EXCL sibling、fsync 后以读取时的 id/storage key/state/deletedAt 做 DB CAS，竞争删除只留下 orphan 给阶段5 GC。安全 orphan且不能复活 row。原生媒体先完成 durable upload 再把目标 Canvas 已 ready 的同源 resolver URL 写入 scene，Core guard 阻止 `data:`/`blob:`/远程或同 Space 跨 Canvas URL 进入 canonical/ledger。`CanvasImportService` 从单一 approved document 字段集派生 root 门禁，使获准 gradient/opacity/background image presentation 可 export→import；其中 `backgroundImageSrc` 与节点媒体共用 URL/asset rebinding 门禁。document/ROOT/node/Frame allowlist 的所有保留值递归剥离 Core 状态，全部外部 ID 重映射并归一隐藏 root；SVG sanitizer 拒绝 active/external content。阶段3由 `src/canvas/canvasSelectionSnapshot.ts` 从 canonical scene 冻结选区（逐元素 `{id, revision}`，canonical JSON 按 Unicode code-point 排序），`src/messages/messageExecutionBinding.ts` 预检 eligible executor 并持久化 `bindingSource`，`src/messages/postMessageCommandParse.ts` 解析单个或有序多个 Canvas selection/binding/structuredMentions，`src/canvas/canvasChatPresentation.ts` 是消息与 API 共用的 canvasContext / canvasContexts 投影；`MessagePosting` 同一事务按序写入全部 snapshot、canvas module context refs、binding 与 required delivery；`src/context/canvasObjectSnapshotResolver.ts` 经注册表接入 Context Assembler 与 Turn Inspector。阶段4由 `src/canvas/canvasAccessGrant.ts`、`canvasGatewayTools.ts`、`canvasToolOps.ts`、`canvasAgentTools.ts`、`canvasSceneSummary.ts`、`canvasMutationFeedback.ts`、`canvasIntentGate.ts`、`canvasSkills.ts` 与 `canvasOutputArtifacts.ts` 提供 durable grant、typed Agent 工具契约、ToolOps 映射、Gateway 工具、skill pack 与 reply artifact 绑定；`TurnCapabilityService.prepare()` 派生 grant 与 canvas scopes，`renewAttempt` 同步 live grant expiry，Agent/Canvas 删除复用 `revokeCanvasAccessGrantsFor*InTransaction`。非空选区 grant 默认含 `import` 但不含 `set_canvas_background`；`asset_import` 只导入 turn/message 绑定的本地附件到授权 Canvas 的 `CanvasAssetStore`。同 Canvas 多 grant 必须带 `snapshotId`；局部 `reorder_nodes` 只授权被排序目标。typed create 在 ROOT 下新增子节点时，Grant 允许伴随的 `structure:root` / 旁系 `parent:` 影响，并把本 turn 新创建的 element/Frame id 扩进同一 grant，供后续 update/delete。

Canvas 是新的深 Module，不是 `WorkspaceFrame`、Renderer Redux 或 Agent runtime 的附属状态。阶段3依赖方向为 Human REST 与 React adapters → `src/canvas/` Core/Import/Asset 边界，以及 MessagePosting / Context Assembler 的窄 Selection Snapshot seam；阶段4把 MCP/CLI thin tools、`CanvasAccessGrant` 和 Agent capability 接到同一 `CapabilityGateway`/`CanvasCore`，生产入口受 `KITH_CANVAS_AGENT_EXECUTION` 保护（`pnpm run desktop:dev` 经 `src/desktop/processCommands.ts` 为 Core/Worker 注入开发开关；packaged 默认 fail-closed）。Transport 只处理认证、限额、解析和序列化；业务 mutation、asset 生命周期、revision、幂等和冲突都位于 `src/canvas/`，不得继续扩大 `src/server/core.ts` 的职责。

Recombyn RCB/editor/nodes/chrome 以 `web/src/features/canvas/upstream/` 的 source island 迁入并保留内部 UI；feature-local Redux 只保存未提交手势、乐观交互投影与缓存，不能维护或恢复 canonical history。已提交 Ctrl-Z/Ctrl-Y 必须向 Core 提交新的 revert/forward mutation，Yjs undo 路径不迁入。所有 durable 写进入 `CanvasCore.apply()`；顶栏改名经 host `kith:canvas-title` 提交 `metadata.rename`，空草稿不发送（`safeTitle` 要求 1–160 字符，`web/src/features/canvas/host/canvasTitleCommit.ts`），以免 Core 拒绝后把默认名填回输入框；其内部从 operation schema 推导 read/write/root/order/asset set，并在同一 workspace SQLite transaction 中完成 metadata/document/element/Frame/structure 冲突校验、apply、revision bump、mutation ledger 与 per-Canvas realtime sequence。Human 使用全局唯一 client operation id；Agent 经 Gateway `turn_operations` 与同一 Core operationId 幂等。Renderer ACK 只表示已投影，不能成为业务 commit。

阶段3每个 Space 新增 `canvas_selection_snapshots` 与 `message_execution_bindings`；阶段4再新增 `canvas_access_grants` 与 `turn_output_artifacts`，并为 binding 持久化 `binding_source`。SceneDocument 作为不含内联大媒体的 canonical JSON 保存；aggregate revision 用于 snapshot/reconnect，mutation ledger 中服务端派生的资源 read/write set 用作 element/Frame/structure 细粒度 CAS，允许同一 base revision 上互不相交的资源提交。Canvas 资产使用 `<spaceRoot>/.kith/canvas-assets/<canvasId>/<assetId.ext>`，不复用聊天附件的 `.kith/uploads` 生命周期；路径只由已注册 Space root、Canvas id 和服务端生成的 asset id 解析，调用方不能提交任意文件路径。资产 reachability 包含当前 scene 与可由 retained inverse 恢复的历史；当前尚无有界 retention/checkpoint，因此历史存储和曾引用资产的回收会线性增长，属于未通过的长期性能/生命周期门禁。
...
Human 圈选后由 Core 从 canonical scene 冻结不可变 `CanvasSelectionSnapshot`；普通 `MessageContextSnapshot` 只引用一个 snapshot id，注册式 `ContextObjectSnapshotResolver` 遍历 turn 全部 bound message 的规范 refs 后再冻结。频道可见消息与执行者分离：server-owned `MessageExecutionBinding` 与 snapshot/message/ref/required delivery 在同一 MessagePosting transaction 写入；eligible executor 必须未删除、v2、有当前 surface access 且实时拥有 `message:send`，DM 从对端推导，频道/话题可用 Composer 选择器或结构化 `@Agent`，并持久化 `bindingSource`。其他 active Agent 也不获 optional wake，且不能用伪造正文 mention 改变 surface。空选区/加号菜单整板授权冻结整张画布的有界 snapshot，并签发整板 live 读/写/create grant（`createParents` 含 `ROOT` 与全部 Frame，含 `set_canvas_background`）；选中 Frame 时写范围限制在该 Frame 内（`createParents` 不含 `ROOT`，`FOCUS_FRAME_ID` 指向该 Frame）。`CanvasAccessGrant` 只能从 binding + bound delivery 派生并逐调用重验 canvas/object/action/expiry，Gateway 不信任请求体自报 scope。Agent 通过 `canvas.scene_summary/skill_list/skill_get/create_frame/create_text/create_shape/create_image/video_generate/update_node/delete_nodes` 以及兼容用的 `snapshot_get/elements_get/elements_apply/context_bundle_create/asset_import/export` 调用同一 Module；MCP 与 CLI 只做 thin transport，业务实现只在 Gateway/Canvas Core。`create_image` 接受已有且属于当前 Canvas 的 `assetId`，或 `genPrompt` 排队 Doubao 生成；`create_lottie`/`create_icon` 仍只接受 assetId。拒绝 remote URL/data URL 与跨 Canvas 资产。`canvas.video_generate` 不是 mutation 工具，返回 jobId。`create_frame` 自定义 id 不能是 `ROOT`，也不能与已有 element 或 Frame 冲突。CLI typed tools 覆盖 create_text/shape/image 与 update_node 的常用字段；Gateway 仍接受 `update_node.patch`，CLI 不解析 JSON patch。mutation 关联既有 `turn_operations`，`TurnReplyCommand.outputRefs` 经 `turn_output_artifacts` 关联本 turn 已提交 mutation；mutation 成功而 reply 前崩溃时，恢复 attempt 以原 operation key reconcile 后补 reply。自然语言 edit/question/read/export 由 Canvas skill 要求 Agent 自己判断，ContextAssembler 不把正则分类写入 `mutationRequired=yes`。Canvas skill pack 含 Frame-first 操作协议、`CANVAS_DESIGN_PRINCIPLES` 与 `CANVAS_SKILLS_CATALOG`；验证失败以 `CanvasToolError`（`invalid_fill`/`invalid_stroke`/`missing_gradient_end`/`missing_required_param`/`skill_not_found`）返回，上一轮失败会在下一轮注入 `LAST_CANVAS_ERROR`。`canvas.scene_summary` 在既有 JSON 上增加 `contextText`（SCENE_FRAMES / SCENE_NODES / FOCUS_FRAME_ID / GRANT / AVAILABLE_FONTS）供模型阅读。`AVAILABLE_FONTS` 来自 `src/canvas/fonts/fontsCatalog.ts` 的 46 族 jsDelivr 目录（`src/canvas/fonts/fontsCatalog.ts:27`），编辑器经 Stage One `fontsListFontsEndpoint` 注入同一目录并由既有 `injectFontFaces` 注册 `@font-face`（`web/src/features/canvas/adapters/recombynStageOneServices.ts:53`）。设计技能 Markdown 位于 `src/canvas/skills/`，打包 Desktop 经 `KITH_SPACE_CANVAS_SKILLS_DIR` 指向 extraResources `canvas-skills`。`canvasIntentGate` 仅在有显式 Agent intent 时作后置 telemetry（默认 `unknown`，`enforcedOnReply=false`）；当前没有 finish/intent 工具，因此 `turn.reply` 不做自然语言硬拦截，服务端只校验 Grant/action、mutation 是否提交、以及 outputRefs 绑定。

外部 Scene JSON 不得直接替换 canonical document。阶段2的 Kith-owned `CanvasImportService` 负责 format/version/schema/大小/深度/数量门禁、节点与 Frame id 唯一性、parent/children/stack/active 引用闭合、全部外部 ID 重映射和单隐藏 root 归一；外部资产在没有同次安全 rebinding 时统一拒绝，不能引用现有 Canvas 或 Space 资产。验证后的 scene 通过 Core create + document operation 的单一 transaction 建立新 Canvas，不走 raw scene 覆盖捷径。

MCP `2026-07-28` 不作为 Canvas 前置。MVP 继续使用当前 broker-backed stdio MCP 和 CLI fallback。modern core 的无 `initialize/initialized` 与 Streamable HTTP 的无 `Mcp-Session-Id` 是不同层次的变化；二者都不替代 Kith application-level broker handle，也不提供 Canvas 需要的 ACL、幂等、事务、revision、冲突和 undo。`x-kith-session-handle` 单独无权，仍需当前 activation 与 worker generation 才能解析 turn claims。SDK v2 + dual-era 仅在外部 host、多 Core 实例或 SDK 1.x 维护终止等条件出现后独立迁移。

完整模块契约、数据模型、UI island、Prompt/Skill 适配、5个开发阶段、失败模式和验收矩阵见 `../archive/specs/2026-08-15-recombyn-canvas-workspace-design.md`；决策摘要见 `../archive/adr/0038-adopt-recombyn-canvas-module.md`。
