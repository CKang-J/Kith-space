...
2026-07-22形成、2026-07-23修订的[系统级可替换Memory Advisor Provider提案](./superpowers/specs/2026-07-22-system-memory-advisor-provider-design.md)：目标是把自动提炼从聊天Agent runtime解耦；新安装默认使用Desktop内置、精确锁版的Pi SDK Provider，Claude Code可切换，并把执行Provider与结构化记忆的Model Profile独立版本化。Human可手工选择模型供应商/模型/API/endpoint/thinking/凭据来源，或显式、安全地从Pi CLI全局配置导入模型目录；项目`.pi`、命令表达式和动态资源发现不进入Advisor。它尚未实现；当前仍只有Claude Agent支持自动advisor，Codex/opencode只可使用runtime-neutral的结构化recall与Human管理。该提案不命名为P-A10.8，也不吞并受限consolidation、skill reconciliation和runtime security/approval/Vault；后三者继续分别后置为P-A11、P-A12、P-S1。
