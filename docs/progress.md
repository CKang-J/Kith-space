...
- **P-A10.0–P-A10.7 已完整实现并通过全新 Desktop/Web 真实验收**：workspace schema v8在v7 episodic memory上增加advisor settings/job/proposal/recall observation及session checklist/snapshot/compaction revision；app.db保持v3。Claude restricted maintenance以无工具/MCP/CLI、ephemeral cwd的独立Port完成eligible turn提取，typed actor/evidence、exclude/secret、source cap/ACL、suppression、dedupe、成本/批次/lease验证后才写active/proposed；provider返回与最终事务再次CAS，candidate、proposal、relation和mutation原子提交；Codex/opencode maintenance诚实为unsupported。Agents记忆面板已提供Structured/Files、active/proposals/archived、服务端过滤/分页、revision/evidence/relation/disclosure/source revoke、accept/reject、edit/correct、archive/restore、retain independent、delete、forget+suppress与advisor freshness。Core启动和周期恢复器封闭post-commit调度丢失窗口；snapshot按session/generation/checksum/64KiB门禁恢复，checklist/short wake跨restart使用单调revision，preview按250ms合并；Codex可映射compaction事件并由持久marker触发下一Envelope，Claude/opencode不伪造能力。真实场景覆盖fresh Human→Desktop设置开启本机Web→生成临时Token→7777登录、全新Space/3个Claude Agent、公开/私有频道、DM/话题、跨surface continuity与中文/英文recall、三响应模式/@all/明确任务、多Agent reply/cede/retry、MCP/CLI、turn详情、记忆生命周期、60秒wake和snapshot/restart。真实运行发现并根治了turn admission identity漏接`source=turn`、v2 task被legacy wake双消费并误停Agent，以及usage event未进入terminal三条问题；最终对抗复审继续关闭Advisor并发/事务、compaction消费、wake/checklist revision、durable recovery与撤权记忆保留窗口。

## 一、现在在哪

- 主干：`main`。临时工作分支不作为阶段进度记录。
- 已完成：P0-P3 后端；P4 单窗口 ChatOnly / Split / ModuleOnly 生产壳；任务模块“全部任务/频道任务”范围侧栏。
- 当前阶段：**P-A10.0–P-A10.7 已完成代码、迁移、文档、43场景矩阵、自动化和最终Desktop/Web真实验收，等待提交**。P-A11 consolidation、P-A12 skill reconciliation、P-S1 sandbox/approval/Vault与H5跨Space编排继续是独立后续。P-A9 事实见 `docs/superpowers/specs/2026-07-18-desktop-modular-monolith-architecture-design.md`，P-A10 实施契约见 `docs/superpowers/specs/2026-07-19-agent-harness-session-context-memory-tools-design.md`。
...
- 最终 unpacked Desktop fresh smoke 以全新隔离数据目录运行并 Exit 0：Core/Worker ready，内置 Web/Drizzle/setup status 可用，`app.db` 成功创建，`kith-space.cmd --help` 可运行；退出后残留受管进程为 0，7777/5273 监听为 0。另一次 packaged Core 真实初始化创建唯一 Human 与 `Home`；当时生成的 workspace.db 为 19 张产品表 + `__drizzle_migrations` 共 20 张物理表、`PRAGMA user_version=2`，并完成优雅退出；当前连接时会依次升级为 schema v8，app.db升级为v3。
