...
- **P-A10.0–P-A10.5 已过自动化门，下一步是 P-A10.6 Advisor 与记忆面板**：workspace schema v7新增canonical/revision/evidence/relation/tag/suppression/mutation/lexical+FTS，app.db v3用独立表族承载Human-only `user_global` memory。Human API提供actor域幂等create、CAS lifecycle、历史revision/relation审计与suppression解除；secret和权威exclude lineage拒绝持久化，replacement relation让旧词查询返回新item。continuity与query recall对两库使用统一score breakdown，支持英文、CJK 2/3-gram与2字exact，并逐次解析message/turn/file当前生命周期、跨Space membership/父级ACL及validity window；任一evidence失效即暂停整项。forget在`secure_delete`连接上删除正文/FTS并truncate WAL，Agent full reset/delete清除其private payload与suppression但保留Space shared。混合evidence取最严格projection，Human consume-once grant与发送事务都会再次核对当前source access。Context Envelope冻结revision/HMAC/projection/score/evidence并在forget后只显示tombstone；`knowledge:read`签发Agent `memory.recall/get`到同一MCP/CLI Gateway。自动advisor、记忆面板和自动capture尚未实现，三家provider live smoke与compaction telemetry仍留给后续门。

## 一、现在在哪

- 主干：`main`。临时工作分支不作为阶段进度记录。
- 已完成：P0-P3 后端；P4 单窗口 ChatOnly / Split / ModuleOnly 生产壳；任务模块“全部任务/频道任务”范围侧栏。
- 当前阶段：**P-A10.0–P-A10.5 已完成，正在进入 P-A10.6 Advisor 与记忆面板**。P-A9 事实见 `docs/superpowers/specs/2026-07-18-desktop-modular-monolith-architecture-design.md`，P-A10 实施契约见 `docs/superpowers/specs/2026-07-19-agent-harness-session-context-memory-tools-design.md`。
...
- 最终 unpacked Desktop fresh smoke 以全新隔离数据目录运行并 Exit 0：Core/Worker ready，内置 Web/Drizzle/setup status 可用，`app.db` 成功创建，`kith-space.cmd --help` 可运行；退出后残留受管进程为 0，7777/5273 监听为 0。另一次 packaged Core 真实初始化创建唯一 Human 与 `Home`；当时生成的 workspace.db 为 19 张产品表 + `__drizzle_migrations` 共 20 张物理表、`PRAGMA user_version=2`，并完成优雅退出；当前连接时会依次升级为 schema v7，app.db升级为v3。
...
2. P-A10.0–P-A10.5 已过自动化门；下一步按规格进入P-A10.6的restricted advisor与记忆面板，不提前把P-A10.7 snapshot/compaction telemetry或P-A11 consolidation混入。H5、Rust试验、生产力模块和P-S1安全升级继续按各自前置关系推进。
...
- P-A10.5 当前全量unit为794通过、11个平台条件skip、0失败；typecheck、完整integration、Desktop build与production bundle（Web 2643 modules、24项MCP工具）通过。隔离定向fixture覆盖workspace v2–v6→v7、app.db v1/v2→v3、required index/FK/journal、actor域幂等/CAS、authoritative exclude、message/turn/file lifecycle、跨Space ACL、validity、replacement pointer、跨库统一score/总配额、无词面continuity、中文2字/英文FTS、Context冻结/tombstone、grant精确action/consume及撤权竞态、Agent删除生命周期，以及workspace/app.db主库+WAL物理canary；既有1万memory基线四类FTS median p95为0.0045–0.0048 ms。P-A10.6 advisor/面板尚未实现；最终`desktop:dev` / 7777真实UI及provider live smoke仍须在P-A10.6–P-A10.7全部落地后按完整场景矩阵执行，不能用当前自动化代替。
