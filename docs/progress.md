...
- **P-A10.0–P-A10.4 已过自动化门，下一步是 P-A10.5 Episodic Memory core**：`kith-core` stdio MCP与`kith-space` CLI现通过同一broker client、canonical strict command schema和`CapabilityGateway`调用Core领域实现，提供context refresh、server-owned reply/cede、progress/turn inspector、ACL约束的conversation read/search、Task全链路、surface checklist、short wake与capability describe。JSON POST先做1 MiB有界解析再实时授权，所有领域写在最终SQLite事务内重验activation、attempt lease、session generation、当前Agent scope及surface/父级ACL；缺少`message:send`的required turn在runtime admission前失败。reply支持按turn/activation/surface/owner绑定的临时附件，25 MiB超限整批失败，1小时过期行和文件/DB崩溃orphan由持久状态与启动/调度清扫恢复。later-query追加turn audit并旋转freshness watermark，不改写原Envelope，跨private/DM disclosure domain在P-A10.5前只给`ref_only`。Task写经窄port复用既有Module并用确定性message ID+operation状态处理崩溃reconciliation。Worker在外部runtime前对MCP做真实stdio/list-tools探针，失败时仅在受控CLI可执行时降级，两路不可用返回`mcp_bootstrap_failed`；实际MCP/CLI调用更新attempt诊断。wake key按turn命名空间，manual start只返回不含正文的逐surface inbox summary。真实Gateway fixture已同时跑MCP、CLI client与CLI parser；三家bootstrap在最终Desktop live smoke前诚实标`fixture_v2`，tool isolation、cwd relocation和compaction telemetry仍为unsupported。revisioned episodic memory、disclosure/suppression、continuity/中文FTS recall与P-A10.6–P-A10.7 UI仍未实现。

## 一、现在在哪

- 主干：`main`。临时工作分支不作为阶段进度记录。
- 已完成：P0-P3 后端；P4 单窗口 ChatOnly / Split / ModuleOnly 生产壳；任务模块“全部任务/频道任务”范围侧栏。
- 当前阶段：**P-A10.0–P-A10.4 已完成，正在进入 P-A10.5 Episodic Memory core**。P-A9 事实见 `docs/superpowers/specs/2026-07-18-desktop-modular-monolith-architecture-design.md`，P-A10 实施契约见 `docs/superpowers/specs/2026-07-19-agent-harness-session-context-memory-tools-design.md`。
...
2. P-A10.0–P-A10.4 已过自动化门；下一步只进入P-A10.5的revisioned episodic memory、disclosure/suppression与continuity/中文FTS recall，不跳到advisor或记忆管理UI。H5、Rust试验、生产力模块和P-S1安全升级继续按各自前置关系推进。
