...
- **P-A10.0–P-A10.1 已过门，正在进入 P-A10.2 durable delivery/turn 纵切**：workspace compatibility 使用不可变 per-version manifest，并在迁移前区分 legacy/future、校验 Drizzle journal hash/前缀与 v6 关键 index/FK；app.db 使用 `user_version=1`、checksum journal 与单事务 runner。workspace schema v6 新增 `agent_harness_state/runtime_sessions`，v5 migration 显式保留旧 `agents.session_id`，不回填任何 per-surface engine session。`SessionModule` 强制 `legacy→migrating→v2` drain 门、按 runtime/model/config/adapter/host/workspace fingerprint 增长 generation；stable broker 只在匹配 attempt activation 期间可用；Worker host 分开 active turn/resident process 上限、同 Agent 串行并 LRU evict。Claude/Codex/opencode v2 bridge 已归一化 session changed、completion、usage、process cancel 与 critical event ack；MCP bootstrap、tool isolation、cwd relocation和compaction telemetry继续诚实标 unsupported。legacy Agent API/Worker event/reconnect/start 已由 harness mode互斥护栏阻止双消费。P-A10.2 落地前产品消息仍全部保持 legacy，尚无 Agent 实际切到 v2。

## 一、现在在哪

- 主干：`main`。临时工作分支不作为阶段进度记录。
- 已完成：P0-P3 后端；P4 单窗口 ChatOnly / Split / ModuleOnly 生产壳；任务模块“全部任务/频道任务”范围侧栏。
- 当前阶段：**P-A10.0–P-A10.1 已完成，正在进入 P-A10.2 durable delivery、logical turn/attempt 与最小 reply/cede 纵切**。P-A9 事实见 `docs/superpowers/specs/2026-07-18-desktop-modular-monolith-architecture-design.md`，P-A10 实施契约见 `docs/superpowers/specs/2026-07-19-agent-harness-session-context-memory-tools-design.md`。
...
2. P-A10.0–P-A10.1 已过门；下一步只进入 P-A10.2 的 durable delivery、logical turn/attempt、operation/output 与最小 reply/cede，不跳到自动记忆或 UI。H5、Rust试验、生产力模块和P-S1安全升级继续按各自前置关系推进。
...
- P-A10.1 当前全量 unit 为 704 通过、11 个平台条件 skip、0 失败；typecheck、28 项 migration/三 adapter/session/broker targeted test、完整 integration、Web build（2641 modules）与 Desktop build 均通过。最终 `desktop:dev` / 7777 真实 UI 验收必须在 P-A10.2–P-A10.7 全部落地后按完整场景矩阵执行；P-A9 的 packaged/browser 基线仍是上一完成门事实。旧 `publicNavContract` 随 public landing 路线一起删除，不再接受把它列为可忽略失败。
