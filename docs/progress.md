...
- **P-A10.0–P-A10.2 已过门，下一步是 P-A10.3 server-owned thread 与 Context Envelope**：workspace/app.db migration、per-surface session、三家 Runtime v2 bridge 与 stable broker 地基保持不变。workspace schema v6 的第二个合法 journal 前缀现扩展为 34 张产品表，加入 delivery、logical turn/attempt/event、operation/output/input mapping、capability activation、context/disclosure/checklist/wakeup 结构；v6 的 P-A10.1 journal 前缀仍可继续迁移。Message/Task/Agent output 在同一 SQLite 事务写 durable delivery，observe/self/system 直接终态并用有界 join 只推进连续 source frontier；task assignee以外的v2成员也会得到terminal observe row。每条 actionable delivery 仍幂等绑定原有 dispatch reservation/wake budget。Core scheduler 按 surface 绑定 turn、claim generation lease、同步续租 broker claims，并以安装级Space FIFO队列和每Space每批最多8个不同Agent轮转；Worker 的admitted+activated总量上限128，未activate admission 120秒过期，同Agent排队可精确取消。Worker逐事件等待ACK，event有64KiB单条、2000条/8MiB聚合上限、terminal预留与truncation摘要；terminal会重传到Core幂等ACK，Core generation推进会清理旧admission/preparing/running/terminal，不能永久占槽。`turn context/reply/cede` 通过loopback Gateway校验activation/lease/input scope，reply原子写message/mention/dispatch chain+depth/output/obligation/cursor且实时事件携带mention，stdout/preview不能成为消息。受支持runtime的新Agent直接形成Human-Agent DM required introduction turn；既有Agent先关闭legacy HTTP admission并等待在途请求drain，再按membership cursor backfill/cutover，失败可停在migrating后幂等续跑，rollback窗口在任何Worker/Core副作用前预检。stop/reset把attempt置cancelled并重新开放未结输入；inactive或runtime配置漂移会退休旧turn并由可用的新session generation重绑，不制造5ms热重试。Gateway逐调用实时membership/父话题ACL、完整MCP、Context Envelope、顶层mention server-owned thread、结构化记忆和UI面板仍未实现，均是后续切片门。

## 一、现在在哪

- 主干：`main`。临时工作分支不作为阶段进度记录。
- 已完成：P0-P3 后端；P4 单窗口 ChatOnly / Split / ModuleOnly 生产壳；任务模块“全部任务/频道任务”范围侧栏。
- 当前阶段：**P-A10.0–P-A10.2 已完成，正在进入 P-A10.3 server-owned thread、Context Envelope 与 turn inspector**。P-A9 事实见 `docs/superpowers/specs/2026-07-18-desktop-modular-monolith-architecture-design.md`，P-A10 实施契约见 `docs/superpowers/specs/2026-07-19-agent-harness-session-context-memory-tools-design.md`。
...
- 最终 unpacked Desktop fresh smoke 以全新隔离数据目录运行并 Exit 0：Core/Worker ready，内置 Web/Drizzle/setup status 可用，`app.db` 成功创建，`kith-space.cmd --help` 可运行；退出后残留受管进程为 0，7777/5273 监听为 0。另一次 packaged Core 真实初始化创建唯一 Human 与 `Home`；当时生成的 workspace.db 为 19 张产品表 + `__drizzle_migrations` 共 20 张物理表、`PRAGMA user_version=2`，并完成优雅退出；当前连接时会依次升级为 schema v6。
...
2. P-A10.0–P-A10.2 已过门；下一步只进入P-A10.3的server-owned顶层mention话题、Context Envelope与turn inspector，不跳到自动记忆。H5、Rust试验、生产力模块和P-S1安全升级继续按各自前置关系推进。
...
- P-A10.2 当前全量unit为736通过、11个平台条件skip、0失败；typecheck、完整integration、Web build（2641 modules）和Desktop build通过。P-A10临时SQLite基线重跑的20-Agent fan-out median p95为2.034ms（仍不是200万delivery长期容量替代证据）。定向覆盖migration/cutover与rollback acceptedAt、durable transaction、task observe/frontier、dispatch reservation、turn lease与capability续租、cancel/requeue、operation幂等、mention实时事件+chain/depth、逐输入obligation、Space FIFO、admission TTL/总量上限、preview truncation/critical reserve、terminal strict usage/lease/重传ACK、generation推进、prepare/shutdown与配置漂移。最终`desktop:dev` / 7777真实UI验收仍须在P-A10.3–P-A10.7全部落地后按完整场景矩阵执行；不能用当前自动化代替。
