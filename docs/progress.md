...
- **P-A10.0–P-A10.3 已过自动化门，下一步是 P-A10.4 Capability Gateway 与 MCP bootstrap**：Human/Agent在顶层频道的direct mention会把root、唯一thread、合法参与者和逐Agent delivery原子提交；`@all`仍留在父频道，silent只加入而不wake。root delivery的cursor owner保持父频道，执行surface固定为thread，Agent后续mention只可把已有父级访问权的成员加入同一thread；v2 `turn.reply`走相同server-owned语义，mixed cutover中v2目标只写durable delivery、legacy目标与output同事务写dispatch reservation并由既有guard执行/重启扫描恢复，migrating目标由cutover backfill承接。`ContextAssembler`在logical turn首次attempt冻结root、root前8条/最多4000估算token的parent as-of snapshot、current batch、continuity恢复层、task/attachment/UI有界snapshot、三层文件记忆索引ref、多source watermark、预算与omission；只注入已绑定frontier，8k required溢出在bind时保留尾部给下一turn。删除source后turn inspector显示HMAC tombstone，omitted/ref-only不冒充已注入正文。app.db v2新增稳定安装级content HMAC key且v1原位迁移。Gateway、Context assembly和最终output事务都会重验当前surface与普通thread父级membership；Human移除或Agent自助leave统一撤销普通child membership、session、wakeup、turn/attempt和capability，task_scoped是带task ref/到期时间的唯一例外并在release/reassign/终态、admission前自然到期或运行中heartbeat越界时同步撤销。Agent持久消息工具栏已提供“上下文/步骤/用量/结果”审计抽屉。完整MCP、later-query refresh、episodic memory与P-A10.5–P-A10.7 UI仍未实现。

## 一、现在在哪

- 主干：`main`。临时工作分支不作为阶段进度记录。
- 已完成：P0-P3 后端；P4 单窗口 ChatOnly / Split / ModuleOnly 生产壳；任务模块“全部任务/频道任务”范围侧栏。
- 当前阶段：**P-A10.0–P-A10.3 已完成，正在进入 P-A10.4 Capability Gateway 与 MCP bootstrap**。P-A9 事实见 `docs/superpowers/specs/2026-07-18-desktop-modular-monolith-architecture-design.md`，P-A10 实施契约见 `docs/superpowers/specs/2026-07-19-agent-harness-session-context-memory-tools-design.md`。
...
2. P-A10.0–P-A10.3 已过自动化门；下一步只进入P-A10.4的完整MCP/CLI Gateway、权威查询和structured stale refresh，不跳到自动记忆。H5、Rust试验、生产力模块和P-S1安全升级继续按各自前置关系推进。
...
- P-A10.3 当前全量unit为753通过、11个平台条件skip、0失败；typecheck、完整integration、Desktop build与Web build（2643 modules）通过。P-A10临时SQLite基线重跑的20-Agent fan-out median p95为2.034ms（仍不是200万delivery长期容量替代证据）。新增定向覆盖原子direct-mention thread、silent/@all、Agent output/thread mixed-mode mention、legacy public autojoin、silent/历史watermark不预留wake budget、actionable legacy crash recovery、reservation后task scope到期/父ACL撤销的原子退款、Context Envelope as-of/bound frontier/8k拆批/HMAC tombstone、app.db v1→v2、逐调用父级ACL与Human/Agent撤权、task_scoped release/reassign/终态/自然到期、stale output和turn inspector injection state。最终`desktop:dev` / 7777真实UI验收仍须在P-A10.4–P-A10.7全部落地后按完整场景矩阵执行；不能用当前自动化代替。
