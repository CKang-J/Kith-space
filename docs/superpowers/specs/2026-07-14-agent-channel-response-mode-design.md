...
“+”菜单通过 portal 渲染，但以整个 Composer 外框定位：默认左边界和宽度都与输入框一致；只在输入框宽度超出视口安全范围时按左右 `8px` 边距收缩。菜单使用上下 `4px`、左右 `5px` 内边距与 `16px` 外圆角；菜单项固定 `30px` 高、`10px` 圆角和 `4px 10px` 内边距。菜单只使用浅色边框和圆角，不使用投影。
