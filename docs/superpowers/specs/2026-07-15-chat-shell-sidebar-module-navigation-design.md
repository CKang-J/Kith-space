...
> **2026-08-16 工作区面板可见性补充：** Chat 标题栏提供右侧模块面板的展开/收起控制。该控制只改变面板可见性，不关闭或移除已打开的工作区标签；再次展开时继续显示原标签及其内容。当没有标签页时，右侧面板显示与标签栏“+”菜单共用同一模块注册表和 Home 过滤规则的直接入口，点击后再创建并打开对应标签页。
...
> 2026-07-25 后续覆盖：功能 UI 仍保持统一无衬线体系，但字体栈已改为内置 Sora Variable 负责英文、数字和拉丁标点，中文回退到系统 `PingFang SC` / `Microsoft YaHei`；代码与路径继续使用系统等宽字体。下方 Inter/纯系统字体栈仅保留为本规格实施时的历史基线。
