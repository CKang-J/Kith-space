...
发行同样服从这条边界：仓库只构建 Desktop production bundle、Windows unpacked 包和安装器，不再维护 Docker、公共 server/daemon/npm 或独立 Web 发布。当前 Windows NSIS 构建链已经可复现，但本地/CI 产物默认未签名；代码签名与真实安装/卸载验收是公开分发前必须完成的发行工作，不能用“成功生成 `.exe`”替代。
...
**Canvas 是 Chat 的视觉工作面，不是生图附属页。** 2026-08-15 已确认以 Apache-2.0 的 Recombyn RCB 编辑器为基础：同一 Space 可打开多张独立无限画布，承载文字、形状、媒体和手绘；Human 可把选区冻结为上下文交给一个明确 Agent，Agent 的修改回写画布并在真实 Chat 中留下可审计回执。首版先完成原生编辑、多 Canvas 标签、选区发 Chat 和单 Agent revisioned 回写；便签、链接、文件、语义思维导图/流程图、多 Agent 区域以及 AI 视频/音频生成按后续能力逐层增加。Kith Harness 和 Core Module 始终是运行与数据权威，不引入第二套画布专用 runtime。

**Chat 与模块共享一个可调工作窗口。** Chat 是默认且保持挂载的基础工作面；打开任务、画布、日历等模块后，右侧 Workspace Tab 与 Chat 并排，两区比例可拖拽，标签按 Space 恢复。Chat 的当前界面状态会成为 agent 可理解的结构化上下文。这是“一边对话、一边做事”的工作手感，不恢复第二套 Dock 或让模块暗中替换会话。
...
- **不做多真人和远程 agent 主机。** 一个安装实例只有一个 Human；全部 agent 只由本机唯一的 Local Runtime Worker 执行。底座遗留的邀请、RBAC、机器加入和远程 daemon 能力已经删除，而不是休眠。
