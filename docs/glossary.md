...
**三护栏**
: 因默认 autopilot 而强制的三项可控性保障，缺一不可，均落在 server 唤醒环这一收口处：**分派深度上限**（限制 agent→agent 逐层再分派的最大深度）、**每任务 token 预算**（单任务用量超限即熔断该分支）、**一键急停**（用户随时停止全空间所有 agent）。与右栏实时轨迹（可见性）配套，共同支撑"默认自动、始终可见可停"。
...
**双壳**
: 顶层两个界面壳，切换发生在"进入/退出一个空间"。**空间总览态**是全局 bento 驾驶舱（"我的整个 OS"）；**空间内部态**以群聊为 C 位（"我正身处的那一场协作"）。进入空间时左导航收起，屏幕让位给协作。

**空间总览态**
: 跨空间聚合的门面，是可拖拽 bento 卡片的正确归宿。v1 薄版只做三块：空间列表、全局收件箱、简单聚合待办。

**空间内部态**
: 进入某空间后的界面：左细图标条 + 中心群聊 + 右栏模块容器。

**群聊 C 位**
: 空间内部态中心恒为群聊，不降级为卡片、不降级为可切换模块。这是刻意选择——协作发生在对话里，若让 bento 看板占 C 位，产品会滑向"生产力看板 + 附带聊天"，丢掉协作灵魂。

**右栏模块化容器**
: 空间内部态右侧的容器，自带底部 dock，切换显示不同模块（任务/日历/文件/实时轨迹/画布等）。DM 不进右栏。

**dock**
: 右栏底部的图标条，用于切换右栏显示哪个模块。注意与左细图标条区分——后者是空间态收起后的左侧竖直导航，刻意不做成第二条底部 dock 以免与右栏 dock 打架。

**agent 实时轨迹**
: 右栏的一个模块，近实时展示 agent 的执行动作序列，是配合默认 autopilot 的透明度窗口。它重要但地位与其他模块平等，不是右栏的固定身份。

**模块提升**
: 吃宽度的模块（画布/日历）可从右栏提升到中心乃至全屏，此时群聊让位为侧边条。对应"右栏扫一眼、中心专注做"的可调性。

**bento 驾驶舱**
: 空间总览态呈现为可拖拽 bento 卡片的个人 OS 首页。bento 只在总览态，被刻意排除在空间内部态 C 位之外。
