...
2026-08-15 Recombyn Canvas Workspace 阶段 1 AgentDock 纠偏实现已完成并通过主任务最终复审；阶段 2 未开始。开发态 `?__canvas_stage1=1` 直接运行固定 `reference/recombyn@abd8198` Git object 的原生 `EditorPage`/RCB/nodes/chrome/panels：320 项闭包逐文件审计，318 项 materialize，2 个未核清品牌二进制排除；重放不依赖嵌套仓库当前 checkout。AgentDock、开关与占位已移除，底栏相邻图片/视频生成器按钮保留 `A` / `Shift+A`，节点编辑与刷新恢复可用；生成提交明确 unavailable，媒体文件只形成本地 data URL，Recombyn request/upload/media Job transport 不进入物化闭包。正式 Workspace Tabs、Canvas Library、SQLite/Canvas Core、资产闭环和 Agent 写回仍不可用。完整方案见 [`Recombyn Canvas Workspace 设计`](./docs/superpowers/specs/2026-08-15-recombyn-canvas-workspace-design.md)。
