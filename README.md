...
基于 Helio Desktop 的本机实测，P-A10 Agent Harness v2 已进入实施。P-A10.0–P-A10.5 已完成migration、Runtime v2、per-surface session、durable delivery/turn、server-owned direct-mention thread、可审计Context Envelope、实时父级ACL/撤权、broker-backed `kith-core` MCP/CLI Gateway，以及revisioned episodic memory。workspace schema v7与app.db v3现提供Human-only CAS管理、forget+suppress、source ACL、continuity+中英文FTS recall、跨surface disclosure projection与consume-once grant；Agent可经`memory.recall/get`读取，Context Envelope冻结revision/HMAC/projection。Claude/Codex/opencode provider live smoke、自动advisor、记忆面板、snapshot/compaction telemetry仍留给后续切片和最终Desktop验收。
