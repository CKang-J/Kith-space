...
2026-08-18 Recombyn Canvas Workspace 阶段3已实现 Canvas 与现有 Chat 上下文联动。Human 可通过原生 Add to Chat 把当时选区冻结为不可变 Selection Snapshot，发到现有 DM/频道/话题；Composer 显示可移除 chip，发送成功后变为不可变引用。workspace schema v13 新增 `canvas_selection_snapshots` 与 `message_execution_bindings`；MessagePosting 同一事务写入 snapshot、context ref、required binding 和 executor required delivery。阶段2的正式 Canvas Library/resource tabs、Canvas Core、细粒度冲突、undo/redo、本地资产与受限导入保持有效。Stage1 两个 SHA 和 `upstream/**` 字节保持不变。阶段4的 Canvas Access Grant、Gateway/MCP、Agent写回和真实AI生成仍未进入；retained history/asset reachability、物理资产 GC 与约3.5 MB Canvas chunk 是未通过的长期门禁。完整方案见 [`Recombyn Canvas Workspace 设计`](./docs/superpowers/specs/2026-08-15-recombyn-canvas-workspace-design.md)。
