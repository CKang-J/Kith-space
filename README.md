...
Chat 壳层与侧栏模块导航已再次收敛：左侧常驻栏纵向显示“图标 + 文字”的模块入口，不显示 Chat 或底部 Dock；Spaces、Inbox、Tasks、Agents 在右侧复用 Chat 的同一主卡片槽位切换，左侧栏不再隐藏，也不再出现 Split 第二工作面。Settings 使用独立模态层。中心 Chat 的圆角、间隙、配色和消息内容轨道保持不变；案例展示继续保持退役。完整边界见 [`Chat 壳层与侧栏模块导航设计`](./docs/superpowers/specs/2026-07-15-chat-shell-sidebar-module-navigation-design.md)。
...
- 常驻侧栏模块入口、单主卡片切换、Settings 模态层与 Dock/案例展示退役：[`2026-07-15-chat-shell-sidebar-module-navigation-design.md`](./docs/superpowers/specs/2026-07-15-chat-shell-sidebar-module-navigation-design.md)
