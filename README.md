...
基于 Helio Desktop 的本机实测，P-A10 Agent Harness v2 已进入实施。P-A10.0 已完成版本感知 migration/app.db runner、Runtime/Harness codec 与容量/中文检索基线；P-A10.1 已加入按 Agent 互斥的 harness state、per-surface session generation、stable broker activation、三家 Runtime v2 bridge 与 Worker session host；P-A10.2 已落地 durable delivery、logical turn/attempt/lease、逐输入 obligation、dispatch reservation、server-owned reply/cede 与最小 broker-backed CLI Gateway。Claude/Codex/opencode 新 Agent 直接从 Human-Agent DM introduction turn 进入 v2，既有 Agent 通过显式 drain/backfill cutover；legacy/v2 仍按 Agent 互斥。Context Envelope/完整 MCP、结构化记忆和管理 UI 尚未启用。
...
- per-surface session、Context Envelope、结构化记忆与 MCP/CLI Gateway 提案：[`2026-07-19-agent-harness-session-context-memory-tools-design.md`](./docs/superpowers/specs/2026-07-19-agent-harness-session-context-memory-tools-design.md)
