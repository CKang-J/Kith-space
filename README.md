...
基于 Helio Desktop 的本机实测，P-A10 Agent Harness v2 的P-A10.0–P-A10.7已完成代码、迁移、文档、自动化与全新Desktop/Web真实验收。当前具备Claude Code、Codex、OpenCode、Pi四家Runtime v2、per-surface session、durable delivery/turn、可审计Context Envelope、broker-backed MCP/CLI Gateway、revisioned episodic memory、restricted advisor、snapshot与compaction telemetry。workspace schema v12在v11之上增加Human-only Canvas document/mutation/asset持久化；app.db v10继续承载安装级模型供应商/模型配置/运行器控制面、三态默认绑定、Agent跨安装确认快照、runtime epoch屏障和外观设置。
...
2026-08-15 Recombyn Canvas Workspace 阶段2已完成并通过主任务最终复核。正式 Canvas Library/resource tabs、workspace schema v12、SQLite Canvas Core、服务端派生的细粒度资源冲突、幂等/undo/redo、per-canvas realtime、Canvas-local assets 与受限 SVG/Scene JSON 导入已落地；安全软删除、每次读取完整性、逐 Frame CAS、Import 保留状态剥离和离线删除重连已有定向回归。Stage1 320项闭包、318项 materialize、两个 SHA 和 `upstream/**` 字节保持不变。Selection Snapshot、Chat、Gateway/MCP、Agent写回和真实AI生成仍未进入；retained history/asset reachability、物理资产 GC 与约3.5 MB Canvas chunk 是未通过的长期门禁。完整方案见 [`Recombyn Canvas Workspace 设计`](./docs/superpowers/specs/2026-08-15-recombyn-canvas-workspace-design.md)。
