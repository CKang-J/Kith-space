...
try {
  ...
  const staleClient = await api(home.id, "POST", "/api/agents", {
  ...
  const listed = await api(home.id, "GET", "/api/agents");
  ...
  const detail = await api(home.id, "GET", `/api/agents/${created.body.id}`);
  ...
  const stalePatch = await api(home.id, "PATCH", `/api/agents/${created.body.id}`, { machineId: "legacy" });
  assert.equal(stalePatch.status, 400);

  const models = await api(home.id, "GET", "/api/local-runtime/models/codex");
  assert.equal(models.status, 200);
  assert.ok(models.body.models.some((model: any) => model.id === "gpt-5.4"));

  const retiredMachines = await api(home.id, "GET", `/api/spaces/${home.id}/machines`);
