...
test("the installation-local Worker cannot be pointed at a remote Core Service", () => {
  ...
  assert.match(workerEntry, /case "agent:start": admitWorkerCommand\(msg\)/);
  assert.match(workerEntry, /rejected Worker command without admission identity/);
  assert.match(workerEntry, /type: "worker:admission"/);
