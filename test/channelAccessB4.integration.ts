...
const ts = Date.now();
const fixture = integrationDatabase("channel-access-b4");
...
async function setup() {
  ...
  const [srv] = await db.insert(schema.servers).values({ id: serverId, name: "TB4", slug: `tb4-${ts}`, ownerId, rootPath }).returning();
  serverId = srv!.id;
