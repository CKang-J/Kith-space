...
echo "→ stopping kith-space prod daemon…"
pkill -f "ENV_FILE=.env.prod tsx src/daemon/index.ts" 2>/dev/null || true   # the launcher shell
pkill -f "kith-space/node_modules.*tsx.*src/daemon/index.ts" 2>/dev/null || true  # its tsx/node child (this checkout only; not worktrees / other repos)

echo "✅ prod down (server + kith-space daemon)."
