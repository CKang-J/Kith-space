...
export const memoryAdvisorSettings = sqliteTable("memory_advisor_settings", {
  ...
  approvedProviderRevision: integer("approved_provider_revision"),
  approvedModelProfileRevision: integer("approved_model_profile_revision"),
  approvedProviderEpoch: integer("approved_provider_epoch"),
  approvedEgressDigest: text("approved_egress_digest"),
  consentEpoch: integer("consent_epoch").default(0).notNull(),
  consentPurpose: text("consent_purpose"),
  consentSourceScope: text("consent_source_scope_json", { mode: "json" }).$type<{ public: boolean; private: boolean; dm: boolean }>(),
  consentAt: timestamp("consent_at"),
  consentActorId: text("consent_actor_id"),
  installationIdentityDigest: text("installation_identity_digest"),
  providerEpochMirror: integer("provider_epoch_mirror"),
  updatedAt: timestamp("updated_at").default(now).notNull(),
});
...
export const memoryAdvisorJobs = sqliteTable("memory_advisor_jobs", {
  ...
  providerEpoch: integer("provider_epoch"),
  installationIdentityDigest: text("installation_identity_digest"),
  executionSnapshot: text("execution_snapshot_json", { mode: "json" }).$type<Record<string, unknown>>(),
  executionSnapshotDigest: text("execution_snapshot_digest"),
  capabilityDigest: text("capability_digest"),
  policyVersion: integer("policy_version"),
  agentConsentEpoch: integer("agent_consent_epoch"),
  sourceScopeDigest: text("source_scope_digest"),
...
export const advisorProviderRuns = sqliteTable("advisor_provider_runs", {
  id: id("id").primaryKey(),
  spaceId: text("space_id").notNull().references(() => spaces.id, { onDelete: "cascade" }),
  agentId: text("agent_id").notNull().references(() => agents.id, { onDelete: "cascade" }),
  status: text("status").$type<"leased" | "running" | "succeeded" | "failed" | "blocked" | "cancelled">().notNull(),
  providerRevision: integer("provider_revision").notNull(),
  modelProfileRevision: integer("model_profile_revision").notNull(),
  providerEpoch: integer("provider_epoch").notNull(),
  consentEpoch: integer("consent_epoch").notNull(),
  installationIdentityDigest: text("installation_identity_digest").notNull(),
  executionSnapshotDigest: text("execution_snapshot_digest").notNull(),
  egressPlan: text("egress_plan_json", { mode: "json" }).$type<Record<string, unknown>>(),
  egressDigest: text("egress_digest").notNull(),
  policyVersion: integer("policy_version").notNull(),
  workerGeneration: integer("worker_generation"),
  batchJobIds: text("batch_job_ids_json", { mode: "json" }).$type<string[]>().notNull(),
  usage: text("usage_json", { mode: "json" }).$type<Record<string, unknown>>(),
  ...
  due: index("advisor_provider_runs_status_idx").on(t.status, t.createdAt),
  agent: index("advisor_provider_runs_agent_idx").on(t.agentId, t.createdAt),
}));
