...
- `docs/superpowers/specs/2026-08-15-recombyn-canvas-workspace-design.md` — **Recombyn Canvas Workspace 规格**：阶段1–3已实现；保留Recombyn RCB内部UI，正式Workspace Tabs、Canvas Core/SQLite、本地资产与 Selection Snapshot/Chat 上下文联动已接入，Canvas Access Grant、Gateway/MCP 和 Agent 写回留给阶段4。
...
当前数据版本：workspace schema v13在v12 Human-only Canvas document/mutation/asset 之上增加 `canvas_selection_snapshots` 与 `message_execution_bindings`；app.db v10在v9安装级三作用域字体与12–16px UI字号之上增加全局 light / dark / system 颜色模式。下方较早的v9/v5描述仅是历史基线。
...
- 数据层：**SQLite**。每 Space 一个 `<folder>/.kith/workspace.db`；中央 `app.db` 保存唯一 Human、稳定 Home 身份、Space registry、Web 模式、访问 Token 哈希/版本、浏览器会话、Desktop设置、安装级外观设置、Human-only user-global episodic memory、安装级Advisor Provider控制面，以及模型供应商、模型配置和runtime profile。当前workspace schema v13在v12 Human-only `canvas_documents / canvas_mutations / canvas_assets` 之上增加 `canvas_selection_snapshots` 与 `message_execution_bindings`；app.db当前为v10。Canvas canonical scene、revision、ledger与资产状态归workspace SQLite所有，文件位于Space内`.kith/canvas-assets`。阶段3把选区冻结为不可变 Selection Snapshot，并与 Chat message context ref、server-owned `MessageExecutionBinding` 和 executor required delivery 同事务写入；`canvas_access_grants` 与 `turn_output_artifacts` 仍属阶段4，当前不存在。`agents.session_id`仍只作互斥legacy rollback来源。app data 默认 `~/.kith-space`，默认 Space 容器为 `~/Kith-space`，Home 为 `~/Kith-space/Home`；`KITH_SPACE_HOME` 只覆盖 app data，`KITH_SPACE_SPACES_DIR` 独立覆盖开发/测试默认 Space 容器。P-A7 H2 已把 Claude Code、Codex、opencode、Pi 的 cwd 切到所属 Space root，把 Agent Memory 放入 `<space>/.kith/agents/<agentId>`，把 adapter 临时状态和Pi generation留在app data runtime目录；Agents 详情的“记忆”文件浏览器只读取当前 agentMemoryDir。Copilot/Kimi/Cursor 仍为 experimental adapter并暂用runtime state cwd。H3/H4的Space root与Home边界不变。v2–v12合法workspace前缀与app.db v1–v9会按immutable manifest/journal迁移到当前版本；workspace migration SQL固定为LF，兼容层只额外接受历史Windows CRLF checkout产生的逐文件精确hash，未知journal仍拒绝；更旧legacy或future schema明确拒绝。
