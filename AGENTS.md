...
- `docs/superpowers/specs/2026-08-15-recombyn-canvas-workspace-design.md` — **Recombyn Canvas Workspace 规格**：方案已确认、尚未实现；保留Recombyn RCB编辑器内部UI，接入现有Workspace Tabs、Kith Harness、Selection Snapshot与revisioned Canvas Module，按5个可运行开发阶段交付MVP并标明后续能力。
...
- `docs/decisions.md` — 记录全部锁定决策；决策 38 已接受 Recombyn Canvas Module 方案但尚未实现，同时保留推理、权衡和**被推翻/修正的决策**演化脉络。理解"凭什么这样定"看这里。
