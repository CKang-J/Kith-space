...
- 测试：内置 `node:test`（`src/**/*.test.ts`、`test/**`）。`pnpm test --unit` 跑单测、`pnpm test --integration` 跑集成、`pnpm run typecheck` 类型检查。测试 runner 会同时把 `KITH_SPACE_HOME` 与 `KITH_SPACE_SPACES_DIR` 指向随机临时 profile。最近一次已接受的完整门禁基线为937通过、11个平台条件skip、0失败；2026-07-25 Windows兼容审计实跑为947通过、0 skip、6失败，其中5项属于跨平台进程/路径/测试缺口，1项为既有CSS契约，详见`docs/cross-platform-compatibility.md`。当前不能表述为全绿，改动须配套运行相关测试并如实记录未通过项。
