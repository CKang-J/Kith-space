...
- `docs/superpowers/specs/2026-08-15-recombyn-canvas-workspace-design.md` — **Recombyn Canvas Workspace 规格**：阶段1–4已实现；保留Recombyn RCB内部UI，正式Workspace Tabs、Canvas Core/SQLite、本地资产、Selection Snapshot/Chat 联动、Access Grant、Gateway/MCP/CLI 与 Agent 写回已接入；阶段4之上已补 Agent 原生 typed Canvas tools（`scene_summary` / `create_*` / `update_node` / `delete_nodes`），Phase 0 再补 `update_frame` / align/distribute/reorder / group/ungroup / duplicate / flip / `boolean_op` / `set_canvas_background`、完整样式参数、`model_hint` 描述与 `LAST_CANVAS_ERROR`。自然语言 edit/question/read/export 由 Canvas skill/Agent 自己判断，服务端不把正则分类写成 `mutationRequired=yes`，`turn.reply` 尚无 finish 硬拦截。生产入口受 `KITH_CANVAS_AGENT_EXECUTION` 保护，阶段5硬化与真实 smoke 仍待后续。
