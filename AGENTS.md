...
当前数据版本：workspace schema v10增加Agent模型绑定、跨安装确认快照和runtime epoch；app.db v8在v6供应商/模型/runtime控制面上增加安装级三作用域字体设置，并允许界面字体在无衬线与等宽分组中选择。下方较早的v9/v5描述仅是本轮实现前的历史基线。
...
- 测试：内置 `node:test`（`src/**/*.test.ts`、`test/**`）。`pnpm test --unit` 跑单测、`pnpm test --integration` 跑集成、`pnpm run typecheck` 类型检查。测试 runner 会同时把 `KITH_SPACE_HOME` 与 `KITH_SPACE_SPACES_DIR` 指向随机临时 profile。2026-07-25 三端兼容收口后的Windows完整单测为962通过、1个平台条件skip、1失败（总计964）：审计时5项平台相关失败已全部修复，唯一失败是本分支外既有的`.sliding-tabs` CSS契约；typecheck、完整integration和production bundle通过。远端Ubuntu/Windows/macOS矩阵仍以实际CI结果为准，不能在未运行前表述为三端全绿。改动须配套运行相关测试并如实记录未通过项。
