...
当前数据版本：workspace schema v15在v14 Canvas Access Grant/`turn_output_artifacts` 之上增加 `canvas_generation_jobs` 异步图像/视频生成任务表；app.db v11在v10全局 light / dark / system 颜色模式之上增加 `generation_providers`（AES-256-GCM 加密的 Doubao/Seedance 等生成凭据）。下方较早的v9/v5描述仅是历史基线。
...
- 数据层：**SQLite**。每 Space 一个 `<folder>/.kith/workspace.db`；中央 `app.db` 保存唯一 Human、稳定 Home 身份、Space registry、Web 模式、访问 Token 哈希/版本、浏览器会话、Desktop设置、安装级外观设置、Human-only user-global episodic memory、安装级Advisor Provider控制面，以及模型供应商、模型配置和runtime profile。当前workspace schema v15在v14 `canvas_access_grants` / `turn_output_artifacts` 之上增加 `canvas_generation_jobs`；app.db当前为v11。Canvas canonical scene、revision、ledger与资产状态归workspace SQLite所有，文件位于Space内`.kith/canvas-assets`。阶段3把选区冻结为不可变 Selection Snapshot，并与 Chat message context ref、server-owned `MessageExecutionBinding` 和 executor required delivery 同事务写入；阶段4从 binding+bound delivery+frozen snapshot 派生 durable `CanvasAccessGrant`，经 Capability Gateway/MCP/CLI thin tools（typed `scene_summary/skill_list/skill_get/create_*/update_node/delete_nodes` 与 Phase 0 排版工具，以及兼容用 `elements_apply`）读回写，并以 `turn_output_artifacts` 关联已提交 mutation；`create_image` 接受本 Canvas `assetId` 或 `genPrompt`（异步 Doubao 图像 Job，立即返回 jobId），落节点时写入与工具栏上传相同的 durable `attrs.src`（`/api/canvas-assets/{spaceId}/{canvasId}/{assetId}`）；`canvas.video_generate` 走 Seedance 视频 Job（规格内部 id 仍为 `seedream`）；真实 Ark 图像 API 是同步 `POST /images/generations`，Worker 把结果 URL 记为 `ark-url:` 前缀以便重启后下载；`arkFetchJson` 默认 90s 超时，避免图生图阻塞整条 Job 队列；视频走 `POST/GET /contents/generations/tasks`。凭据存在 app.db `generation_providers`，AES-256-GCM，master key 位于 `<appData>/master.key`（0o600 / Windows owner ACL），也接受 `KITH_CANVAS_DOUBAO_API_KEY` / `KITH_CANVAS_SEEDREAM_API_KEY` / `KITH_CANVAS_ARK_API_KEY`。Settings「图像与视频」只填一把方舟 Key，PATCH `name=ark` 同时写入 `doubao`/`seedream` 两行。生成器模型目录为 Seedream 4.0/4.5/5.0 与 Seedance 1.0 Pro/Lite（`src/canvas/generation/arkModelCatalog.ts`），Job `config` 带 `model`/`resolution`，不支持的分辨率/时长在切换模型时禁用。Core 启动 `GenerationSupervisor` 按 Space 轮询 Job。Human 工具栏图像/视频生成器走 `POST /api/canvases/:canvasId/generation-jobs`（无 Access Grant），placement.`targetNodeId` 让 Worker 把生成板就地 promote 为 image/video（写 durable `attrs.src` 并清 `imageGenerator`/`videoGenerator`）；Agent 路径无 target 时仍 `create_image`/`create_video` 新节点。选中图片工具条的放大/去背景/多角度（以及扩展/换字）走同一 Job，`placement.skipNodeCreate` 只导入结果资产，由现有 process clone 写入 durable `attrs.src`；Doubao 在有 `referenceImage` 时提交 Ark `image` data URL。Human Job 入队后立即 kick Worker。橡皮是本地涂抹后 Canvas-asset 上传；标记是按住拖拽框选后飞入左侧 Chat Composer（裁切附件 + 图片节点 selection-to-chat grant；框选矩形冻进 snapshot 只给 Agent；拖选用 overlay 屏幕矩形换算视口坐标）；去背景模式菜单留在选区工具条内；图片分层/`detectRegions` 明确未接入（需要独立视觉分解，不恢复 Recombyn `/api/v1/image/process`）。Agent ToolOp `image_process` 仍延后。`GET /api/canvases/:canvasId/generation-jobs/:jobId` 供前端轮询。Phase 3.1 队列骨架之上 Phase 3.2/3.3 已接入真实供应商适配器；真实 Desktop “星空海报” smoke 与真实 API Key 联调仍待本机验收。`canvas.scene_summary` 的 `AVAILABLE_FONTS` 来自 `src/canvas/fonts/fontsCatalog.ts` 的 46 族 jsDelivr 目录，编辑器经 Stage One `fontsListFontsEndpoint` 注入同一目录并 `injectFontFaces`；选中 Frame 时 grant 焦点在该 Frame 且 create 使用 frame-local 坐标，加号菜单整板授权或空选中签发整板写 grant；自然语言 edit/question/read/export 由 Canvas skill/Agent 判断，服务端不把正则分类写成 `mutationRequired=yes`；生产入口受 `KITH_CANVAS_AGENT_EXECUTION` 保护。`agents.session_id`仍只作互斥legacy rollback来源。app data 默认 `~/.kith-space`，默认 Space 容器为 `~/Kith-space`，Home 为 `~/Kith-space/Home`；`KITH_SPACE_HOME` 只覆盖 app data，`KITH_SPACE_SPACES_DIR` 独立覆盖开发/测试默认 Space 容器。P-A7 H2 已把 Claude Code、Codex、opencode、Pi 的 cwd 切到所属 Space root，把 Agent Memory 放入 `<space>/.kith/agents/<agentId>`，把 adapter 临时状态和Pi generation留在app data runtime目录；Agents 详情的“记忆”文件浏览器只读取当前 agentMemoryDir。Copilot/Kimi/Cursor 仍为 experimental adapter并暂用runtime state cwd。H3/H4的Space root与Home边界不变。v2–v12合法workspace前缀与app.db v1–v9会按immutable manifest/journal迁移到当前版本；workspace migration SQL固定为LF，兼容层只额外接受历史Windows CRLF checkout产生的逐文件精确hash，未知journal仍拒绝；更旧legacy或future schema明确拒绝。
