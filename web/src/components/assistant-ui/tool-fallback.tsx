...
function ToolFallbackTrigger({
  ...
  const displayLabel = label ?? (isCancelled ? "Cancelled tool" : "Used tool");

  return (
    <CollapsibleTrigger
      data-slot="tool-fallback-trigger"
      className={cn(
        "aui-tool-fallback-trigger group/trigger flex w-fit appearance-none items-center gap-2 border-0 bg-transparent px-0 py-1.5 text-sm text-muted-foreground outline-none transition-[color,scale] hover:bg-transparent hover:text-foreground focus-visible:!outline-none focus-visible:text-foreground focus-visible:underline focus-visible:underline-offset-4 active:scale-[0.98]",
        className,
      )}
      {...props}
    >
...
function ToolFallbackArgs({
  ...
  return (
    ...
    >
      {label ? (
        <p className="aui-tool-fallback-args-header mb-1 text-xs text-muted-foreground">
          {label}
        </p>
      ) : null}
